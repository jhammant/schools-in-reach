...
import { state, matchesPhase, GROUP_META, admissionsFor, hasAdmissionsData, laName } from "./state.js";
...
const filters = { text: "", phase: "", sector: "", gender: "", faith: "", reach: "" };
...
export function renderList(onSelect) {
  ...
  if (state.home) {
    ...
    rows = matching.filter((s) => s.lat != null && (!bounds || bounds.contains([s.lat, s.lon]))).sort((a, b) => a.name.localeCompare(b.name));
  }
  ...
  $("#list-summary").textContent = state.home
    ? `${matching.filter((s) => s._dist != null && s._dist <= 5).length} schools within 5 miles of ${state.home.label}, nearest first${rows.length > MAX_ROWS ? ` (showing the closest ${MAX_ROWS})` : ""}.`
          ...
          return `
            <li class="school-item" data-urn="${s.urn}" tabindex="0">
              ...
              <div>
                ...
                <div class="pills">${reachPill(rec, s._dist, s)}${ofsted}${faith}</div>
              </div>
